...
//   const [viewingRepliesId, setViewingRepliesId] = useState(null);
...
//     try {
//       const res = await axios.get(${API}?communityId=${communityId});
...
//     setViewingRepliesId((prev) => (prev === id ? null : id));
//   };
...
//         <p className={styles.noResults}>No discussions found.</p>
//       ) : (
...
//                   <div className={styles.iconWrapper} onClick={() => toggleReplies(item._id)}>
//                     <AiOutlineEye />
...
//                 {viewingRepliesId === item._id && item.replies?.length > 0 && (
...
//     try {
//       const res = await axios.get(http://localhost:5000/api/communities/${communityId});
...
const DiscussionFeed = () => {
  ...
  const userName = localStorage.getItem("userName");
  const API = "http://localhost:3001/discussion";
  ...
  const fetchCommunityName = async () => {
    try {
      const res = await axios.get(`http://localhost:3001/communities/${communityId}`);
      setCommunityName(res.data.name || "Community");
    } catch (error) {
      console.error("Failed to fetch community name", error);
    }
